...
public class taskViewAdapter extends RecyclerView.Adapter<taskViewAdapter.taskHolder>{
    ...
    public taskViewAdapter(Context context, ArrayList<Task> tasks, char type){
        ...
        if (tasks.size() > 0) {
            if (type == 'O') {
                this.tasks = new ArrayList(viewOverdue(tasks));
            } else if (type == 'D') {
    ...
    //this will handle if the view is for overdue tasks
    @RequiresApi(api = Build.VERSION_CODES.O)
    public ArrayList<Task> viewOverdue(ArrayList<Task> tasks){
        ArrayList<Task> temp = new ArrayList<>();
        System.out.println("Overdue & Day");
        for (int i = 0; i < tasks.size(); ++i){
            if (tasks.get(i).getDate().compareTo(LocalDate.now()) == 0 || tasks.get(i).getDate().isBefore(LocalDate.now())){
    ...
    public ArrayList<Task> viewWeek(ArrayList<Task> tasks){
        ...
        for (int i = 0; i < tasks.size(); ++i){
            if (WeekHelper.getWeek(tasks.get(i).getDate()).compareTo(WeekHelper.getWeek(LocalDate.now())) == 0 && !tasks.get(i).getDate().isBefore(LocalDate.now())){
                temp.add(tasks.get(i));
            }
        }
        return temp;
    ...
    public ArrayList<Task> viewMonth(ArrayList<Task> tasks){
        ...
        for (int i = 0; i < tasks.size(); ++i){
            if (tasks.get(i).getDate().getMonth().compareTo(LocalDate.now().getMonth()) == 0 && !tasks.get(i).getDate().isBefore(LocalDate.now())){
                temp.add(tasks.get(i));
            }
        }
        return temp;
        ...
        holder.taskComplete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (((CompoundButton) view).isChecked()){
                    ...
                    //Is the task that was checked for complete overdue? If so, remove it from
                    //the database.
                    if(tasks.get(pos).getDate().isBefore(LocalDate.now())) {
                        MainActivity.RemoveFromDatabase(tasks.get(pos));
                    }
